...
CREATE TYPE farm_status AS ENUM ('farming', 'farmed', 'claimed');
...
BEGIN
    RETURN QUERY
    SELECT u.invites + 1, u.invited_by, u.tg_id
    FROM users AS u
...
CREATE TABLE users (
  ...
  address VARCHAR(55),
  language VARCHAR(5),
  farm farm_status NOT NULL DEFAULT 'claimed',
...
CREATE TABLE user_tasks (
  ...
  FOREIGN KEY (user_id) REFERENCES users(tg_id) ON DELETE CASCADE ON UPDATE NO ACTION,
...
CREATE TRIGGER check_invitee_inviter_not_null BEFORE INSERT ON public.invites FOR EACH ROW EXECUTE FUNCTION public.enforce_not_nullable_on_insert();
